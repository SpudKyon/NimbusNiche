...
import com.spud.nimbus.product.mapper.CategoryMapper;
...
public class CategoryServiceImpl extends ServiceImpl<CategoryMapper, Category> implements CategoryService {
	...
	public void update(Category category) {
		...
		if (Objects.equals(dbCategory.getCategoryId(), category.getParentId())) {
			throw new NimbusException("分类不能成为本身的上级分类");
		}
		existCategoryName(category);
	...
	public void deleteById(Long categoryId) {
		...
		if (count > 0) {
			throw new NimbusException("该分类在使用中，不能进行删除操作");
		}
	...
	@Override
	@Caching(evict = {
			@CacheEvict(cacheNames = CacheNames.CATEGORY_LIST_BY_PARENT_ID_AND_SHOP_ID,
					key = "#shopId + ':' + #parentId"),
	...
	public void getPathNames(List<CategoryVO> categorys) {
		...
		Map<Long, Category> categoryMap = listByCategoryIds.stream()
	...
	private void setChildCategory(List<CategoryVO> categories, List<CategoryVO> childCategories) {
		...
		Map<Long, List<CategoryVO>> secondCategoryMap = childCategories.stream()
	...
	private void existCategoryName(Category category) {
		...
		if (countByName > 0) {
			throw new NimbusException("分类名已存在，请重新输入");
		}
	}
