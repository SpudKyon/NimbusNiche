...
import com.spud.nimbus.api.product.vo.*;
...
import com.spud.nimbus.product.dto.SpuPageSearchDTO;
...
public class SpuController {
	...
	public Result<PageVO<SpuVO>> platformPage(PageDTO pageDTO, SpuPageSearchDTO spuDTO) {
	...
	public Result<PageVO<SpuVO>> page(PageDTO pageDTO, SpuPageSearchDTO spuDTO) {
	...
	public Result<SpuVO> getBySpuId(@RequestParam Long spuId) {
		// 获取spu信息
	...
	public Result<Void> save(@Valid @RequestBody SpuDTO spuDTO) {
	...
	public Result<Void> update(@Valid @RequestBody SpuDTO spuDTO) {
		checkSaveOrUpdateInfo(spuDTO);
		List<Long> skuIds = spuDTO.getSkuList().stream().filter(sku -> Objects.nonNull(sku.getSkuId()))
				.map(SkuDTO::getSkuId).collect(Collectors.toList());
	...
	public Result<Void> delete(@RequestParam Long spuId) {
	...
	public Result<Void> updateSpuData(@RequestBody SpuDTO spuDTO) {
	...
	public Result<Void> spuChangeStatus(@RequestBody SpuDTO spuDTO) {
	...
	private void spuUpdateStatus(SpuDTO spu) {
		...
		if (StrUtil.isNotBlank(error)) {
			throw new NimbusException(error);
		}
	...
	private void spuBatchUpdateStatus(SpuDTO spu) {
		...
		if (CollUtil.isEmpty(spuList)) {
			throw new NimbusException("您选择的商品信息有误，请刷新后重试");
		}
		...
		if (CollUtil.isEmpty(spuIds)) {
			throw new NimbusException("您所选择的商品中没有符合操作条件的商品");
		}
		spuService.batchRemoveSpuCacheBySpuId(spuIds);
		if (errorList.size() > 0) {
			throw new NimbusException("商品id为：" + errorList.toString() + "的" + errorList.size() + "件商品不符合操作条件");
		}
		spuService.changeSpuStatus(spu.getSpuId(), spu.getStatus());
		spuService.removeSpuCacheBySpuId(spu.getSpuId());
	}
	...
	private void checkSaveOrUpdateInfo(SpuDTO spuDTO) {
		if (!Objects.equals(Constant.PLATFORM_SHOP_ID, AuthUserContext.get().getTenantId())
				&& Objects.isNull(spuDTO.getShopCategoryId())) {
			throw new NimbusException("店铺分类不能为空");
		}
		if (Objects.isNull(spuDTO.getCategoryId())) {
			throw new NimbusException("平台分类不能为空");
		}
