...
import com.spud.nimbus.api.product.bo.EsAttrBO;
...
import com.spud.nimbus.product.model.Spu;
...
public class SpuServiceImpl extends ServiceImpl<SpuMapper, Spu> implements SpuService {
	...
	@Override
	@Caching(evict = { @CacheEvict(cacheNames = CacheNames.SPU_KEY, key = "#spuId"),
	...
	public void changeSpuStatus(Long spuId, Integer status) {
		...
		if (!Objects.equals(status, StatusEnum.ENABLE)) {
			...
			Result<Void> imgRes = indexImgFeignClient.deleteBySpuId(spuVO.getSpuId(), spuVO.getShopId());
			if (!imgRes.isSuccess()) {
				throw new NimbusException("服务异常");
			}
		}
	}
	...
	public void deleteById(Long spuId) {
		...
		if (Objects.isNull(spuVO) || Objects.equals(spuVO.getStatus(), StatusEnum.DELETE.value())) {
			throw new NimbusException("商品不存在或者已被删除！");
		}
		// 删除商品、sku信息(逻辑删除)
		spuMapper.updateStatusBySpuId(spuId);
		skuService.deleteBySpuId(spuId);
		// 把轮播图中关联了该商品的数据删除
		Result<Void> imgRes = indexImgFeignClient.deleteBySpuId(spuId, spuVO.getShopId());
		if (!imgRes.isSuccess()) {
			throw new NimbusException("服务异常");
		}
	}
	...
	public EsProductBO loadEsProductBO(Long spuId) {
		...
		List<SpuAttrValueVO> attrs = spuAttrsBySpuId.stream()
				.filter(spuAttrValueVO -> spuAttrValueVO.getSearchType().equals(1)).collect(Collectors.toList());
	...
	public List<Long> getSpuIdsBySpuUpdateDTO(List<Long> shopCategoryIds, List<Long> categoryIds, Long brandId,
			Long shopId) {
		if (CollUtil.isEmpty(shopCategoryIds) && CollUtil.isEmpty(categoryIds) && Objects.isNull(brandId)
	...
	public PageVO<SpuVO> platformPage(PageDTO pageDTO, SpuPageSearchDTO spuDTO) {
		...
		Result<List<ShopDetailVO>> shopResponse = shopDetailFeignClient.listByShopIds(new ArrayList<>(shopIdSet));
		Map<Long, ShopDetailVO> shopMap = shopResponse.getData().stream()
				.collect(Collectors.toMap(ShopDetailVO::getShopId, s -> s));
