...
import com.spud.nimbus.api.search.vo.search.*;
...
import com.spud.nimbus.search.constant.*;
...
public class ProductSearchManager {
	...
	public List<SpuSearchVO> list(ProductSearchDTO productSearchDTO) {
		...
		try {
			...
			throw new NimbusException("搜索服务出了点小差，请稍后再试", e);
		}
		return spuList;
	}
	...
	private EsPageVO<ProductSearchVO> buildSearchResult(EsPageDTO pageDTO, SearchResponse response) {
		...
		// 总页码
		int totalPages = (int) total % pageDTO.getPageSize() == 0 ? (int) total / pageDTO.getPageSize()
				: ((int) total / pageDTO.getPageSize() + 1);
			...
			for (Terms.Bucket bucket : attrsBuckets) {
				...
				for (Terms.Bucket attrValueBucket : attrLongTerms.getBuckets()) {
					...
					for (SearchHit hit : parsedTopHits.getHits().getHits()) {
						...
						AttrValueSearchVO attrValueSearchVO = Json.parseObject(hit.getSourceAsString(),
	...
	private void agg(ProductSearchDTO param, SearchSourceBuilder searchSourceBuilder, Boolean isAgg) {
		...
		// 按照品牌进行聚合
		TermsAggregationBuilder brands = AggregationBuilders.terms(EsConstant.BRANDS).field(EsConstant.BRAND_ID)
				.size(10);
		...
		if (Objects.isNull(param.getShopId())) {
			TermsAggregationBuilder categories = AggregationBuilders.terms(EsConstant.CATEGORIES)
					.field(EsConstant.CATEGORY_ID).size(10);
			categories.subAggregation(
					AggregationBuilders.terms(EsConstant.CATEGORY_NAME).field(EsConstant.CATEGORY_NAME).size(1));
			searchSourceBuilder.aggregation(categories);
		}
		// 搜索店铺中的商品，按照店铺分类信息进行聚合
		else {
			TermsAggregationBuilder categories = AggregationBuilders.terms(EsConstant.SHOP_CATEGORIES)
					.field(EsConstant.SHOP_CATEGORY_ID).size(10);
			categories.subAggregation(AggregationBuilders.terms(EsConstant.SHOP_CATEGORY_NAME)
					.field(EsConstant.SHOP_CATEGORY_NAME).size(1));
			searchSourceBuilder.aggregation(categories);
		}
		...
		// 按照属性ID进行聚合
		TermsAggregationBuilder attrIds = AggregationBuilders.terms(EsConstant.ATTR_IDS).field(EsConstant.ATTR_ATTR_ID)
				.size(10);
		attrs.subAggregation(attrIds);
		TermsAggregationBuilder attrValueIds = AggregationBuilders.terms(EsConstant.ATTR_VALUE_IDS)
				.field(EsConstant.ATTR_ATTR_VALUE_ID).size(10);
		attrIds.subAggregation(attrValueIds);
		String[] include1 = { EsConstant.ATTR_ATTR_NAME, EsConstant.ATTR_ATTR_VALUE_ID,
				EsConstant.ATTR_ATTR_VALUE_NAME };
		attrValueIds.subAggregation(AggregationBuilders.topHits(EsConstant.TOP_HITS_DATA).fetchSource(include1, null)
				.sort(EsConstant.ATTR_ATTR_VALUE_NAME, SortOrder.ASC).size(1));
	...
	private void keywordSearch(ProductSearchDTO param, BoolQueryBuilder boolQueryBuilder) {
		...
		if (param.getKeyword().length() > 1) {
			// 卖点，不分词
			keywordShouldQuery
					.should(QueryBuilders.matchPhraseQuery(EsConstant.SELLING_POINT, param.getKeyword()).boost(3))
					// 店铺名，不分词
	 ...
	 * 进行排序
	 */
	private void sort(ProductSearchDTO param, SearchSourceBuilder searchSourceBuilder,
		...
		if (Objects.isNull(param.getSort())) {
			...
			// 销量数 log1p
			ScoreFunctionBuilder<FieldValueFactorFunctionBuilder> saleNumScoreFunction = new FieldValueFactorFunctionBuilder(
					EsConstant.SALE_NUM).modifier(FieldValueFactorFunction.Modifier.LOG1P).factor(0.1f);
			...
			FunctionScoreQueryBuilder functionScoreQueryBuilder = QueryBuilders
					.functionScoreQuery(boolQueryBuilder,
							ArrayUtil.toArray(filterFunctionBuilders,
	...
	private void filterQueryIfNecessary(ProductSearchDTO param, BoolQueryBuilder boolQueryBuilder) {
		...
		if (statusList.size() != 0) {
			BoolQueryBuilder should = QueryBuilders.boolQuery()
					.should(QueryBuilders.termsQuery(EsConstant.SPU_STATUS, statusList));
		...
		if (Objects.nonNull(param.getHasStock())) {
			boolQueryBuilder
					.filter(QueryBuilders.termQuery(EsConstant.HAS_STOCK, BooleanUtil.isTrue(param.getHasStock())));
		}
	...
	private void categoryFilterQuery(ProductSearchDTO param, BoolQueryBuilder boolQueryBuilder) {
		...
		if (Objects.nonNull(param.getShopSecondaryCategoryId())) {
			boolQueryBuilder.filter(
					QueryBuilders.termQuery(EsConstant.SHOP_SECONDARY_CATEGORY_ID, param.getShopSecondaryCategoryId()));
		}

		// 平台一级分类
		if (Objects.nonNull(param.getPrimaryCategoryId())) {
			boolQueryBuilder
					.filter(QueryBuilders.termQuery(EsConstant.PRIMARY_CATEGORY_ID, param.getPrimaryCategoryId()));
		}
	...
	public List<SpuSearchVO> limitSizeListByShopIds(List<Long> shopIds, Integer size) {
		...
		shop.subAggregation(AggregationBuilders.topHits(EsConstant.TOP_HITS_DATA).fetchSource(include, null)
				.sort(EsConstant.SALE_NUM, SortOrder.DESC).size(size));
		searchSourceBuilder.aggregation(shop);
		searchSourceBuilder.size(0);
		log.debug("构建的DSL语句 {}", searchSourceBuilder.toString());
		SearchRequest searchRequest = new SearchRequest(new String[] { EsIndexEnum.PRODUCT.value() },
				searchSourceBuilder);
		// 2、执行检索请求
		SearchResponse response = null;
		List<SpuSearchVO> spuList = null;
